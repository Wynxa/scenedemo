...
def htcl_feats_generation(cfg, local_rank, distributed, logger):
    ...
    if is_main_process():
        ...
        if bg_feats_runing is not None and bg_feats_runing.shape[0] > 0:
            upper = save_index + bg_feats_runing.shape[0] - 1
            if max_bg_feats == 0 or upper < max_bg_feats:
                torch.save(bg_feats_runing.clone(), feats_dir + str(save_index) + '_' + str(upper))
