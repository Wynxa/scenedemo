...
def _get_relation_predictor(model):
    module = model.module if hasattr(model, "module") else model
    return module.roi_heads.relation.predictor
...
def htcl_train_sgg_net(cfg, local_rank, distributed, logger):
    ...
    tb_writer = None
    if get_rank() == 0 and cfg.TENSORBOARD.ENABLED:
        mkdir(cfg.TENSORBOARD.LOG_DIR)
        tb_writer = SummaryWriter(log_dir=cfg.TENSORBOARD.LOG_DIR)
        ...
        if tb_writer is not None:
            for loss_name, loss_value in loss_dict_reduced.items():
                tb_writer.add_scalar(
                    "train_losses/{}".format(loss_name),
                    float(loss_value.item()),
                    iteration,
                )
            predictor = _get_relation_predictor(model)
            tb_stats = getattr(predictor, "latest_tb_stats", {})
            for stat_name, stat_value in tb_stats.items():
                if torch.is_tensor(stat_value):
                    scalar = float(stat_value.item())
                else:
                    scalar = float(stat_value)
                tb_writer.add_scalar("conflict_reentry/{}".format(stat_name), scalar, iteration)
            tb_writer.add_scalar(
                "train_meta/lr",
...
class CenterLoss(nn.Module):
    def __init__(self, num_classes=51, feat_dim=1024):
        ...
        self.pred_fre = self._build_pred_frequency_order()
        head_count = min(cfg.MODEL.num_center_head_cls, len(self.pred_fre))
        self.head_pred_idx = torch.tensor(self.pred_fre[:head_count], dtype=torch.long).cuda()
        self.center_head_pred_idx = torch.tensor(self.pred_fre[:head_count], dtype=torch.long).cuda()
        self.body_tail = self.pred_fre[head_count:]
        self.LogSoftmax = nn.LogSoftmax(dim=1)
        self.criterion_loss = nn.CrossEntropyLoss()

    def _build_pred_frequency_order(self):
        if self.num_class == 51:
            return [0, 31, 20, 48, 30, 22, 29, 8, 50, 21, 1, 43, 49, 40, 23, 38, 41, 6,
                    7, 33, 11, 46, 16, 47, 25, 19, 5, 9, 35, 24, 10, 4, 14, 13, 12, 36,
                    44, 42, 32, 2, 26, 28, 45, 3, 17, 18, 34, 37, 27, 39, 15]

        statistics = get_dataset_statistics(cfg)
        rel_counts = statistics['fg_matrix'].sum((0, 1)).cpu()
        fg_indices = list(range(1, min(self.num_class, rel_counts.numel())))
        fg_indices.sort(key=lambda idx: float(rel_counts[idx]), reverse=True)
        return [0] + fg_indices
