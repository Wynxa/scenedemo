...
class HTCL_Feats_Dataset(torch.utils.data.Dataset):
    def __init__(self, cfg, logger=None):
        ...
        self.bg_count = len(np.where(self.rels_labels == 0)[0])
    ...
    def __getitem__(self, index):
        ...
        if rels != 0:
            ...
            chunk_start = 100 * int(bg_id / 100)
            chunk_end = min(chunk_start + 99, self.bg_count - 1)
            feats_grp = torch.load(self.bg_feats_path + str(chunk_start) + '_' + str(chunk_end))
