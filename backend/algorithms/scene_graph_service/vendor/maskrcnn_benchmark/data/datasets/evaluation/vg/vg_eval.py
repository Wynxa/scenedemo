...
def _collect_triplets(gt_classes_list, relationships_list):
    triplets = set()
    for gt_classes, rels in zip(gt_classes_list, relationships_list):
...
def _build_custom_zeroshot_triplets(cfg, dataset):
    if not hasattr(dataset, "roidb_file") or not hasattr(dataset, "gt_classes") or not hasattr(dataset, "relationships"):
        return None

    try:
        train_split_mask, _, train_gt_classes, _, train_relationships = load_graphs(
            dataset.roidb_file,
            'train',
            cfg.DATASETS.debug_num_im,
            num_val_im=cfg.DATASETS.debug_num_val_im,
    ...
    train_triplets = _collect_triplets(train_gt_classes, train_relationships)
    test_triplets = _collect_triplets(dataset.gt_classes, dataset.relationships)
    zeroshot_triplets = sorted(test_triplets - train_triplets)
...
def do_vg_evaluation(
    ...
    zeroshot_triplet = None
    if "relations" in iou_types and cfg.TEST.Eval_all_metrics:
        zeroshot_triplet = _build_custom_zeroshot_triplets(cfg, dataset)
        if zeroshot_triplet is None:
            # Fallback for the original VG setup.
    ...
    if cfg.TEST.Save_eval_results:
        save_output(output_folder, groundtruths, predictions, dataset)

    result_str = '\n' + '=' * 100 + '\n'
    if cfg.TEST.COCO_mAp or output_folder is None:
    ...
    if "relations" in iou_types:
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX and (
            cfg.TEST.Eval_all_metrics or len(result_dict.get(mode + '_accuracy_hit', {}).get(20, [])) > 0
        ):
    ...
    if "relations" in iou_types:
        ...
        if cfg.TEST.Val_mode == 'Recall':
            return float(np.mean(result_dict[mode + '_recall'][100]))
        elif cfg.TEST.Val_mode == 'mRecall':
            return float(np.mean(result_dict[mode + '_mean_recall'][100]))
...
def OIv6_evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    """
    Returns:
        pred_to_gt: Matching from predicate to GT
        pred_5ples: the predicted (id0, id1, cls0, cls1, rel)
