...
def _build_vg_frequency_summary(result_list_100, rel_name_list):
    # VG-specific head/middle/tail buckets only make sense when the custom
    # predicate space is large enough to cover those fixed indices.
    ...
    detail_items = []
    for i in ind_head + ind_middle + ind_tail:
        detail_items.append((str(rel_name_list[i - 1]), float(result_list_100[i - 1])))
...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        ############### using rel_nms in predcls and sgcls ##############
        # ipdb.set_trace()
...
class SGMeanRecall(SceneGraphEvaluation):
    ...
    def generate_print_string(self, mode):
        ...
        freq_summary = _build_vg_frequency_summary(
            self.result_dict[mode + '_mean_recall_list'][100],
            self.rel_name_list,
        )
        if freq_summary is not None:
            ...
            result_str += ' for mode=%s, type=Mean Recall 100.' % mode
            result_str += '\n'
...
class SGNGMeanRecall(SceneGraphEvaluation):
    ...
    def generate_print_string(self, mode):
        ...
        freq_summary = _build_vg_frequency_summary(
            self.result_dict[mode + '_ng_mean_recall_list'][100],
            self.rel_name_list,
        )
        if freq_summary is not None:
            result_str += 'ng-mR100:'
            ...
            result_str += ' for mode=%s, type=No Graph Constraint Mean Recall 100.' % mode
            result_str += '\n'
...
'''
    rel_nms operation, provided by RU-Net and HL-Net. 
'''
