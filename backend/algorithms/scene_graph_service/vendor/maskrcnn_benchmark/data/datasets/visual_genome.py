...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        max_retry = min(len(self.filenames), 20)
        cur_index = index
        for _ in range(max_retry):
            try:
                img = Image.open(self.filenames[cur_index]).convert("RGB")
                if img.size[0] != self.img_info[cur_index]['width'] or img.size[1] != self.img_info[cur_index]['height']:
                    print('='*20, ' ERROR index ', str(cur_index), ' ', str(img.size), ' ', str(self.img_info[cur_index]['width']), ' ', str(self.img_info[cur_index]['height']), ' ', '='*20)

                flip_img = (random.random() > 0.5) and self.flip_aug and (self.split == 'train')
                target = self.get_groundtruth(cur_index, flip_img)
                ...
                return img, target, cur_index
            except (UnidentifiedImageError, OSError, ValueError) as e:
                print("WARNING: skip unreadable image {} ({})".format(self.filenames[cur_index], e))
                if self.split == 'train':
                    cur_index = random.randrange(len(self.filenames))
                else:
                    cur_index = (cur_index + 1) % len(self.filenames)

        raise RuntimeError("Failed to load a valid image after {} retries starting from index {}".format(max_retry, index))
...
def get_OIv6_statistics(train_data, must_overlap=True):
...
def load_image_filenames(img_dir, image_file):
    ...
    for i, img in enumerate(im_data):
        ...
        fns.append(filename)
        img_info.append(img)
    # The original Visual Genome release contains 108073 valid images after
    # filtering corrupted files, but custom datasets can be much smaller.
