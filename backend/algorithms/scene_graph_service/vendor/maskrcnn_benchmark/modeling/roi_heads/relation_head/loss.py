...
class RelationLossComputation(object):
    ...
    def __init__(
        ...
        relation_device = torch.device(cfg.MODEL.DEVICE if hasattr(cfg.MODEL, "DEVICE") else "cpu")
        ...
        2.4253, 0.9280, 1.2198, 0.0563, 0.2921, 0.0862]).to(relation_device)
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits):
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX and cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL:
            # In predcls, object labels are forced to GT one-hot values, so this term
            # becomes a constant logging artifact rather than a learnable objective.
            loss_refine_obj = relation_logits.sum() * 0.0
        else:
            loss_refine_obj = self.criterion_loss(refine_obj_logits, fg_labels.long())

        if cfg.MODEL.reweight_fineloss:
            return {}, loss_refine_obj
